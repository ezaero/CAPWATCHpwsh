...
try {
    ...
    Write-OperationLog "Running monthly account deletion maintenance" "Expired members will be removed from Entra ID"
    
    if (Test-ExecutionMode) {
        Remove-ExpiredMemberAccounts
        Write-Log "Account deletion completed."
    } else {
        Write-Log "[DRY-RUN] Would delete expired member accounts (preview mode - no accounts deleted)"
    }
